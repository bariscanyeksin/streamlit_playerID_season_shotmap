...
st.set_page_config(
    page_title="PlayerID Season Shotmap", initial_sidebar_state="expanded"
)
...
player_id = st.sidebar.text_input(
    "Player ID:", placeholder="737066", value="737066", help="FotMob Player ID"
)
entry_id = st.sidebar.text_input(
    "Season ID:", placeholder="0-0", value="0-0", help="FotMob Season ID"
)

headers = {
    "accept": "*/*",
    "accept-language": "tr-TR,tr;q=0.9,en-US;q=0.8,en;q=0.7",
    "if-none-match": '"14izir9il751huz"',
    "priority": "u=1, i",
    ...
    "x-mas": "eyJib2R5Ijp7InVybCI6Ii9hcGkvcGxheWVyRGF0YT9pZD0xMDM1NjQzIiwiY29kZSI6MTczMzE1ODM3MzgzNiwiZm9vIjoiMDkzZjcyNmY2In0sInNpZ25hdHVyZSI6IkMwRTUyRDc4QkE2RkRDMTQyMUQzRjRCMzQ1NTcxOTlGIn0=",
}
...
if player_id and entry_id:
    ...
    def get_player_data(player_id):
        response = requests.get(
            f"https://www.fotmob.com/api/playerData?id={player_id}", headers=headers
        )

        data = response.json()
        return data

    def fetch_player_season_and_league(player_data, season_id):
    ...
    if player_data:
        ...
        season_string, league_string = fetch_player_season_and_league(
            player_data, entry_id
        )

        def get_team_name_from_season_and_league(data, season_string, league_string):
            for season in data["careerHistory"]["careerItems"]["senior"][
                "seasonEntries"
            ]:
        ...
        team_name, team_id = get_team_name_from_season_and_league(
            player_data, season_string, league_string
        )
        ...
        def get_shotmap_data(player_id, entry_id):
            ...
            shotmap_response = requests.get(shotmap_url, headers=headers)
        ...
        def get_player_match_played_stats(player_id, season_id):
            ...
            if data:
                ...
                match_stats = {"Matches": None, "Started": None, "Minutes": None}
        ...
        if shotmap_data is not None:
            ...
            goal_stat = next(
                (item for item in player_shooting_stats if item["title"] == "Goals"),
                None,
            )
            ...
            shots_stat = next(
                (item for item in player_shooting_stats if item["title"] == "Shots"),
                None,
            )
            shots_count = shots_stat["statValue"] if shots_stat else "-"

            # 'Shots on target' başlığını arayarak isabetli şut sayısını çekme
            shotsontarget_stat = next(
                (
                    item
                    for item in player_shooting_stats
                    if item["title"] == "Shots on target"
                ),
                None,
            )
            shotsontarget_count = (
                shotsontarget_stat["statValue"] if shotsontarget_stat else "-"
            )

            # 'xG' başlığını arayarak beklentili gol sayısını çekme
            xG_stat = next(
                (item for item in player_shooting_stats if item["title"] == "xG"), None
            )
            ...
            xGOT_stat = next(
                (item for item in player_shooting_stats if item["title"] == "xGOT"),
                None,
            )
                ...
                if shot["expectedGoals"] is not None:
                    if shot["eventType"] == "Goal":
                        ...
                        pitch.lines(
                            ...
                            shot["goalCrossedY"],
                            ax=ax_shotmap,
                            color=to_rgba(shot_color, alpha=0.5),
                            lw=1,
                        )
                        pitch.scatter(
                            shot["x"],
                            ...
                            ax=ax_shotmap,
                            c=shot_color,
                            s=round(shot["expectedGoals"], 2) * 800,
                            edgecolors="black",
                            marker="*",
                            alpha=0.8,
                            lw=0.5,
                    ...
                    if shot["eventType"] == "AttemptSaved":
                        ...
                        if shot["isBlocked"] and (
                            not shot.get("expectedGoalsOnTarget", 0)
                        ):
                            pitch.lines(
                                shot["x"],
                                shot["y"],
                        ...
                        elif (not shot["isBlocked"]) and (
                            shot.get("expectedGoalsOnTarget", 0) > 0
                        ):
                            pitch.lines(
                                shot["x"],
                                shot["y"],
                            ...
                            pitch.lines(
                                ...
                                shot["goalCrossedY"],
                                ax=ax_shotmap,
                                color=to_rgba(shot_color, alpha=0.25),
                                lw=1,
                            )
                        pitch.scatter(
                            shot["x"],
            ...
            ax_shotmap.text(
                ...
                0.08,
                "@bariscanyeksin\nData: FotMob",
                transform=ax_shotmap.transAxes,
                fontsize=9,
                fontproperties=prop,
                ha="right",
                va="bottom",
            ...
            url_teamlogo = (
                f"https://images.fotmob.com/image_resources/logo/teamlogo/{team_id}.png"
            )
            response_teamlogo = requests.get(url_teamlogo, headers=headers)
            img_teamlogo = mpimg.imread(BytesIO(response_teamlogo.content))

            # Görseli ekleme
            imagebox_teamlogo = OffsetImage(img_teamlogo, zoom=0.4, alpha=0.5)
            ab_teamlogo = AnnotationBbox(
                imagebox_teamlogo,
                (-0.2, 0.9),
                frameon=False,
                xycoords="axes fraction",
            ...
            bbox = Bbox(
                [[-0.25, 0.1], [0.45, 0.225]]
            )  # Sol alt köşe (0.05, 0.1), sağ üst köşe (0.3, 0.3)
            ...
            for i in range(len(data)):
                ...
                cell.set_facecolor(
                    to_rgba(primary_text_color, alpha=0.15)
                )  # Veri hücreleri arka plan rengi
                cell.set_edgecolor(primary_text_color)  # Hücre kenar rengi
                cell.set_text_props(
                    color=primary_text_color, fontproperties=prop, fontweight="normal"
            ...
            for i in range(len(data)):
                ...
                cell.set_edgecolor(primary_text_color)  # Hücre kenar rengi
                cell.set_text_props(
                    color=primary_text_color, fontproperties=prop, fontweight="bold"
                )  # Yazı rengi ve kalınlığı
            ...
            buf.seek(0)
            file_name = (
                f"{player_name_replaced}_{league_string}_{season_string}_shotmap.png"
            )

            st.download_button(
                label="Download", data=buf, file_name=file_name, mime="image/png"
            )
        else:
        ...
        st.sidebar.markdown(
            "---"
        )  # Add a horizontal line to separate your signature from the content
        ...
        twitter_icon_white_base64 = img_to_base64(
            "icons/twitter_white.png"
        )  # White version of Twitter icon
        github_icon_white_base64 = img_to_base64(
            "icons/github_white.png"
        )  # White version of GitHub icon
